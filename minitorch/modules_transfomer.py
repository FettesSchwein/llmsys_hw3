...
class TransformerLayer(Module):
    ...
    def forward(self, x):
        ...
        x_norm = self.ln_1(x.view(batch_size * seq_len, n_embd)).contiguous().view(batch_size, seq_len, n_embd)
        x = x + self.attention(x_norm)
        
        # 2. Layer Norm -> FeedForward -> Residual Connection (PRE-LN)
        x_norm = self.ln_2(x.view(batch_size * seq_len, n_embd)).contiguous().view(batch_size, seq_len, n_embd)
        x = x + self.ff(x_norm)
        return x
        ### END ASSIGN3_3
