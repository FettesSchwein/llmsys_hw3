...
class SimpleOps(TensorOps):
    ...
    def matrix_multiply(a: "Tensor", b: "Tensor") -> "Tensor":
        """
        Matrix multiplication for SimpleOps.
        Implements batched matrix multiply similar to FastOps but without numba.
        """
        ...
        # If we have more than 3 dimensions, we need to flatten the batch dims
        # and then reshape at the end
        if len(out.shape) > 3:
            ...
            # Reshape to 3D
            a = a.contiguous().view(batch_size, a.shape[-2], a.shape[-1])
            b = b.contiguous().view(batch_size, b.shape[-2], b.shape[-1])
            out_3d = a.zeros((batch_size, out.shape[-2], out.shape[-1]))
            
            # Get storage, shape, strides via tuple()
            a_storage, a_shape, a_strides = a.tuple()
            b_storage, b_shape, b_strides = b.tuple()
            out_storage, out_shape, out_strides = out_3d.tuple()
        else:
            # Get storage, shape, strides via tuple()
            a_storage, a_shape, a_strides = a.tuple()
            b_storage, b_shape, b_strides = b.tuple()
        ...
        # Perform matrix multiplication using manual loops
        # This is slow but maintains gradient tracking
        a_batch_stride = a_strides[0] if a_shape[0] > 1 else 0
        b_batch_stride = b_strides[0] if b_shape[0] > 1 else 0
        
        for i1 in range(out_shape[0]):
