...
def binary_cross_entropy_loss(out, y):
    ...
    log_out = (out + 1e-8).log() 
    log_inv_out = ((ones - out) + 1e-8).log()
    loss = -(y * log_out + (ones - y) * log_inv_out)
    return loss.mean()
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        x_view = x.view(batch, in_size)
        
        w_view = self.weights.value.view(in_size, self.out_size)
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        x = embeddings.mean(1).view(embeddings.shape[0], self.embedding_dim)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                x = minitorch.tensor(x_batch, backend=BACKEND)
                y = minitorch.tensor(y_batch, backend=BACKEND)
